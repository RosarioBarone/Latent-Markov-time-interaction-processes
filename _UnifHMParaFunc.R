  ...
  for(iter in 2:MCMCiter){
    ...
    if(perm!=1){
      ...
      for(i in 1:n){
        Z<-data[[i]]$U
        for (j in 1:S){
          Z[which(Z[,1]==j),1]=permutations[[perm]][[j]]
        }
        data[[i]]$U<-Z 
      }
    }
    ...
    ACCEPT<-exp(sum(lnum)+dnorm(log(eta.prop),0,10,log = TRUE)-log(eta.prop)-sum(lden)-dnorm(log(last.eta),0,10,log=TRUE)+log(last.eta)); 
