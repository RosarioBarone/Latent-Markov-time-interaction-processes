#########################################################################################
#########################################################################################
######     MCMC inference for semi-parametric Time-Interaction Processes           ######
#########################################################################################
#########################################################################################
...
semiparametric_MCMCsamplingTIPs<-function(data,MCMCiter,MCMC.strategy="Sequential",sigma.prop=0.05,random.gamma=TRUE,time.partition=100,c0,eta=1,burn.in=0.5){
  library(mvtnorm)
  library(extraDistr)
  library(truncnorm)
  TT=data[[1]]$TT
  ...
  lambda0<-list(rgamma.process.augm(data=NULL,from=0,to=TT,a=c0,eta=1, time.partition=time.partition))
  ...
  for(iter in 2:MCMCiter){
    ...
    if(Updating=="Joint"){
      ...
      if(psi.prop[3]<psi.prop[4]){ 
        ...
        ACCEPT<-exp(sum(unlist(lnum))+lprior.num-sum(log(psi.prop[2:4]))-sum(unlist(lden))-lprior.den+sum(log(last.psi[2:4])))
    ...
    if(Updating=="Sequential"){
      ...
      ACCEPT<-exp(sum(unlist(lnum))+lprior.num-log(psi.prop[2])-sum(unlist(lden))-lprior.den+log(last.psi[2]))
    ...
    prop.lambda0<-list(rgamma.process.augm(data=NULL,from=0,to=TT,a=c0,eta=eta[iter-1], time.partition=time.partition))
    data.proposal=data
    ...
    if(isTRUE(random.gamma)){
     ...
     ACCEPT<- exp(dgamma.process(post.baseline[iter,,],a=c0,eta =prop, log = TRUE) + dgamma(prop,1,1, log = TRUE) -log(prop)
         - dgamma.process(post.baseline[iter,,],a=c0,eta =last, log = TRUE) - dgamma(last,1,1, log = TRUE) +log(last))  
     if(ACCEPT>runif(1)){
       eta[iter]=prop
     }else{
       eta[iter]=last
