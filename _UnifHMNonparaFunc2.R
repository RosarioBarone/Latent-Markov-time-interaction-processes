...
HMM_nonparametric_MCMCsamplingTIPs<-function(data,MCMCiter,MCMC.strategy,sigma.prop=NULL,time.partition=10,GP.time.partition=100,prior.p=NULL,burn.in=0.15,mu.starting=c(-0.4,1.2),theta.starting=c(0.3,0.5),alpha.starting=c(0.5,3),beta.starting=c(1.4,4),gamma.starting=0.4,eta.starting=1,a=10,S){
 ...
 if(s!=1) lprior.den<-dnorm(last.psi[1,s],mean = 0,sd=1000,log=TRUE)+dgamma(last.psi[2,s],1,100,log = TRUE) +dunif(last.psi[3,s],0,last.psi[4,s],log = TRUE) + dunif(last.psi[4],0,15,log = TRUE)
    ...
    #### Update eta ####
      ...
      ACCEPT<- exp(dgamma.process(post.baseline[iter,,],a=a,eta =prop, log = TRUE) + dgamma(prop,1,1, log = TRUE) 
                   - dgamma.process(post.baseline[iter,,],a=a,eta =last, log = TRUE) - dgamma(last,1,1, log = TRUE) )  
      #### FIXING NUMERICAL PROBLEMS ######
      if(is.na(ACCEPT)|is.nan(ACCEPT)) ACCEPT=0
      if(ACCEPT>runif(1)){
        eta[iter]=prop
